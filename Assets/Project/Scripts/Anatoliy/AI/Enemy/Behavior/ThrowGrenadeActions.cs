...
[Serializable, GeneratePropertyBag]
[NodeDescription(name: "AlwaysThrowGrenadeAtPlayer",
    story: "[Agent] always throws grenade at player",
    category: "Action", id: "d4b32e0ac8f25d9e4b0c6c12f3a85e9b")]
public partial class AlwaysThrowGrenadeAtPlayerAction : Action
{
    [SerializeReference] public BlackboardVariable<GameObject> Agent;
    ...
    protected override Status OnStart()
    {
        ...
        // Единственное отличие от оригинала — нет проверки ShouldThrowGrenade
        if (_grenadeModule == null || !_grenadeModule.CanThrowGrenade)
            return Status.Failure;

        _enemy.Navigation.Stop();
        _grenadeModule.StartWindUp();

        _maxDuration = _grenadeModule.TotalDuration + 1f; // TotalDuration уже есть в модуле
        _elapsed     = 0f;
    ...
    protected override Status OnUpdate()
    {
        ...
        if (_elapsed >= _maxDuration) // страховка если Phase завис
        {
            _grenadeModule.Cancel();
            return Status.Failure;
        }
